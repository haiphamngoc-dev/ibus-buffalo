...
//!
//! This daemon connects to the IBus bus via D-Bus and registers an `IBusFactory` and `IBusEngine`.
//! It intercepts key events from IBus, runs them through the `buffalo_core` engine to support
//! Vietnamese input (Telex, VNI, VIQR), and uses various input methods/modes (e.g. preedit, surrounding text,
//! X11 key simulation) to output the corrected text to focused applications.
...
const IBUS_HYPER_MASK: u32 = 1 << 27;
...
const IBUS_TAB: u32 = 0xff09;
/// Key code representing the Left Arrow key.
const IBUS_LEFT: u32 = 0xff51;
...
fn is_backspace_mode(im: i32) -> bool {
    matches!(
        im,
        SURROUNDING_TEXT_IM
...
fn get_offset_runes(new_text: &str, old_text: &str) -> (String, usize) {
...
async fn gnome_get_focus_window_class() -> Option<String> {
...
async fn get_focus_window_class() -> String {
...
impl IBusEngine {
    ...
    #[zbus(signal)]
    async fn commit_text(signal_emitter: &SignalEmitter<'_>, text: Value<'_>) -> zbus::Result<()>;
    ...
    #[zbus(signal)]
    async fn hide_preedit_text(signal_emitter: &SignalEmitter<'_>) -> zbus::Result<()>;
...
impl IBusEngine {
    ...
    async fn handle_backspace(&self, signal_emitter: &SignalEmitter<'_>, input_mode: i32) -> bool {
    ...
    async fn commit_and_reset(&self, signal_emitter: &SignalEmitter<'_>, input_mode: i32) {
        let preedit = {
            let mut engine = self.engine.lock().unwrap();
            let preedit = engine.get_processed_string(VIETNAMESE_MODE);
            engine.reset();
    ...
    async fn send_backspace(signal_emitter: &SignalEmitter<'_>, im_mode: i32, n: usize) {
        if n == 0 {
            return;
        }
...
/// The main entrypoint of the `ibus-buffalo` daemon.
